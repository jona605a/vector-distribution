...
@constraint(Prob, [n=1:N], sum(Assign[v,n]*(AllVectors[v,"Sex"]=="M") for v=1:V) <= VectorsPerTrip[n]*(1-Femaleratio) + 1 + MoreMales[n])   # Define MoreMales
@constraint(Prob, [n=1:N], sum(Assign[v,n]*(AllVectors[v,"Sex"]=="F") for v=1:V) <= VectorsPerTrip[n]*Femaleratio     + 1 + MoreFemales[n]) # Define MoreFemales
...
@constraint(Prob, [n=1:N], AbsSndTime[n] >= MoreSndTime[n])
@constraint(Prob, [n=1:N], AbsSndTime[n] >= LessSndTime[n])
...
@constraint(Prob, [n=1:N], AbsDrivers[n] >= MoreDrivers[n])
@constraint(Prob, [n=1:N], AbsDrivers[n] >= LessDrivers[n])
...
@constraint(Prob, [n=1:N], sum(Assign[v,n]*(AllVectors[v,"Lyngby/Ballerup"]=="B") for v=1:V) <= HasBallerupVector[n] * VectorsPerTrip[n])
@constraint(Prob, [n=1:N], sum(Assign[v,n]*(AllVectors[v,"Lyngby/Ballerup"]=="B") for v=1:V) >= HasBallerupVector[n] * 2)
